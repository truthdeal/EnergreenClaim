...
// Send native token to an account for gas fees
async function sendNativeToken(from , to, _amount) {
...
function runTest(Holder, ContractName) {

  // Test suite for the EnergreenToken Claim
  describe("EnergreenToken Claim" + ContractName, function () {
    // Run before all tests
    this.beforeAll(async function () {
      [ owner, signer, acc1, acc2, acc3, acc4, acc5, acc6, acc7, acc8, acc9, acc10 ] = await ethers.getSigners();

      // Impersonating account for the holder (Private Sale 1 token account)
      await requestAccount(Holder);
      holder = await signerGetting(Holder);
      console.log("Holder: ", holder.address);

      let TestERC20 = await ethers.getContractFactory("TestERC20");
      let Claim = await ethers.getContractFactory(ContractName);
      ...
      // Transfer some native token to the holder's address
      await sendNativeToken(owner, holder, "1");
    ...
    it("third claim test", async () => {
      ...
      let oldClaimed = await claimInstance.connect(acc1).claimed(acc1.address);
...
// Public
runTest("0x34EcE43178f23F908164651aB673ffbfc26b0b22", "EnergreenClaimPublic");

// Private 1
runTest("0xFaC7c87D1777662909b7Bf7e4E7bc0922423229c", "EnergreenClaimPrivateSale1");

// Private 2
runTest("0x14530Dd3325CE7D035d231210CC4b2bF5b0ebE88", "EnergreenClaimPrivateSale2");
